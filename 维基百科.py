...
# 提取网页内容
content = soup.find('div', {'class': 'mw-parser-output'}).text.strip()
print(f"内容: {content}")
...
for link in unique_links:
    try:
        ...
        # 提取新链接的标题和内容
        new_title = soup.find('h1', id='firstHeading').text.strip()
        new_content = soup.find('div', {'class': 'mw-parser-output'}).text.strip()
        print(f"新链接标题: {new_title}")
        print(f"新链接内容: {new_content}")
